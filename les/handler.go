...
// Package les implements the Light MayaCodemanager Subprotocol.
package les
...
import (
	...
	"github.com/MayaCodemanager/go-maya/common/mclock"
	...
	"github.com/MayaCodemanager/go-maya/light"
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light MayaCodemanager protocol")
	...
	log.Info("Light MayaCodemanager protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light MayaCodemanager peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light MayaCodemanager handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light MayaCodemanager peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light MayaCodemanager message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light MayaCodemanager message arrived", "code", msg.Code, "bytes", msg.Size)
