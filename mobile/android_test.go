...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.MayaCodemanager", new NodeConfig());
			node.start();
			...
			MayaCodemanagerClient ec = node.getMayaCodemanagerClient();
	...
	// signatures too. Regression test for go-maya issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/MayaCodemanager/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.MayaCodemanager", "github.com/MayaCodemanager/go-maya/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "MayaCodemanager", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.MayaCodemanager.gethtest"
