...
import (
	...
	"github.com/MayaCodemanager/go-maya/internal/jsre"
...
type tester struct {
	workspace string
	stack     *node.Node
	MayaCodemanager  *eth.MayaCodemanager
	console   *Console
...
func newTester(t *testing.T, confOverride func(*eth.Config)) *tester {
	...
	// Create a networkless protocol stack and start an MayaCodemanager service within
	...
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return eth.New(ctx, ethConf) }); err != nil {
		t.Fatalf("failed to register MayaCodemanager protocol: %v", err)
	}
	...
	stack.Service(&MayaCodemanager)

	return &tester{
		workspace: workspace,
		stack:     stack,
		MayaCodemanager:  MayaCodemanager,
		console:   console,
		input:     prompter,
		output:    printer,
	}
