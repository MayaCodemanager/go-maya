...
import (
	...
	"github.com/MayaCodemanager/go-maya/accounts/usbwallet/internal/trezor"
...
// or whether the MayaCodemanager app was not started on it.
...
// MayaCodemanager address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.MayaCodemanagerAddress)
	if _, err := w.trezorExchange(&trezor.MayaCodemanagerGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.MayaCodemanagerSignTx{
	...
	response := new(trezor.MayaCodemanagerTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.MayaCodemanagerTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
