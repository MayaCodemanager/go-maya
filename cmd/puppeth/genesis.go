...
import (
	...
	math2 "github.com/MayaCodemanager/go-maya/common/math"
...
// C++ MayaCodemanager implementation.
...
// newAlethGenesisSpec converts a go-maya genesis block into a Aleth-specific
// chain specification format.
func newAlethGenesisSpec(network string, genesis *core.Genesis) (*alethGenesisSpec, error) {
	// Only ethash is currently supported between go-maya and aleth
...
type parityChainSpec struct {
	...
	Genesis struct {
		Seal struct {
			MayaCodemanager struct {
				Nonce   hexutil.Bytes `json:"nonce"`
				MixHash hexutil.Bytes `json:"mixHash"`
			} `json:"MayaCodemanager"`
		} `json:"seal"`
...
// newParityChainSpec converts a go-maya genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only ethash is currently supported between go-maya and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	// https://github.com/MayaCodemanager/EIPs/blob/master/EIPS/eip-608.md
	spec.Params.EIP150Transition = hexutil.Uint64(genesis.Config.EIP150Block.Uint64())

	// Spurious Dragon: 155, 160, 161, 170
	// https://github.com/MayaCodemanager/EIPs/blob/master/EIPS/eip-607.md
	...
	spec.Genesis.Seal.MayaCodemanager.Nonce = (hexutil.Bytes)(make([]byte, 8))
	binary.LittleEndian.PutUint64(spec.Genesis.Seal.MayaCodemanager.Nonce[:], genesis.Nonce)

	spec.Genesis.Seal.MayaCodemanager.MixHash = (hexutil.Bytes)(genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
// pyMayaCodemanagerGenesisSpec represents the genesis specification format used by the
// Python MayaCodemanager implementation.
type pyMayaCodemanagerGenesisSpec struct {
	Nonce      hexutil.Bytes     `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPyMayaCodemanagerGenesisSpec converts a go-maya genesis block into a Parity specific
// chain specification format.
func newPyMayaCodemanagerGenesisSpec(network string, genesis *core.Genesis) (*pyMayaCodemanagerGenesisSpec, error) {
	// Only ethash is currently supported between go-maya and pyMayaCodemanager
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyMayaCodemanagerGenesisSpec{
