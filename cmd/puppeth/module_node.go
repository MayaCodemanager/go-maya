...
// nodeDockerfile is the Dockerfile required to run an MayaCodemanager node.
var nodeDockerfile = `
FROM MayaCodemanager/client-go:latest
...
RUN \
  echo 'geth --cache 512 init /genesis.json' > geth.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.MayaCodemanager/keystore/ && cp /signer.json /root/.MayaCodemanager/keystore/' >> geth.sh && \{{end}}
...
// an MayaCodemanager node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.MayaCodemanager{{if .Ethashdir}}
...
// deployNode deploys a new MayaCodemanager node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.MayaCodemanager"],
