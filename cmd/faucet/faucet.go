...
// This file is part of go-maya.
//
// go-maya is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// go-maya is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with go-maya. If not, see <http://www.gnu.org/licenses/>.
...
import (
	...
	"github.com/MayaCodemanager/go-maya/p2p/discv5"
	"github.com/MayaCodemanager/go-maya/p2p/enode"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the MayaCodemanager protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for MayaCodemanager and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // MayaCodemanager address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an MayaCodemanager light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // MayaCodemanager protocol stack
	client *ethclient.Client   // Client connection to the MayaCodemanager chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the MayaCodemanager light client protocol
	...
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightMayaCodemanager
			ctx.Service(&serv)
...
// close terminates the MayaCodemanager connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the MayaCodemanager address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/MayaCodemanager/go-maya/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and MayaCodemanager address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the MayaCodemanager address and profile URL.
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No MayaCodemanager address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and MayaCodemanager address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain MayaCodemanager address,
