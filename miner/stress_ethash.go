// Copyright 2018 The go-maya Authors
...
func main() {
	...
	for _, node := range nodes {
		...
		if err := node.Service(&MayaCodemanager); err != nil {
			panic(err)
		}
		if err := MayaCodemanager.StartMining(1); err != nil {
			panic(err)
		}
	}
	time.Sleep(3 * time.Second)
	...
	for {
		...
		var MayaCodemanager *eth.MayaCodemanager
		if err := nodes[index%len(nodes)].Service(&MayaCodemanager); err != nil {
			panic(err)
		}
		...
		if err := MayaCodemanager.TxPool().AddLocal(tx); err != nil {
			panic(err)
		}
		nonces[index]++

		// Wait if we're too saturated
		if pend, _ := MayaCodemanager.TxPool().Stats(); pend > 2048 {
...
func makeMiner(genesis *core.Genesis) (*node.Node, error) {
	// Define the basic configurations for the MayaCodemanager node
	...
	// Start the node and configure a full MayaCodemanager node on it
